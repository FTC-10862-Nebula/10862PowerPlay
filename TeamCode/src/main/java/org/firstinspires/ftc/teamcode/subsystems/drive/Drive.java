...
import static org.firstinspires.ftc.teamcode.subsystems.drive.Drive.Constants.*;
...
public class Drive extends MecanumDrive {
    ...
    private double speed = Constants.Drivetrain.Speed.NORMAL_SPEED;
    private double voltage = 12;

    private final TrajectorySequenceRunner trajectorySequenceRunner;

    private final TrajectoryVelocityConstraint VEL_CONSTRAINT = getVelocityConstraint(Controller.MAX_VEL, Controller.MAX_ANG_VEL, Constants.Drivetrain.TRACK_WIDTH);
    ...
    public Drive(OpMode opMode, boolean isUsingImu) {
        super(Controller.kV, Controller.kA, Controller.kStatic, Constants.Drivetrain.TRACK_WIDTH, Constants.Drivetrain.TRACK_WIDTH, Constants.Drivetrain.LATERAL_MULTIPLIER);
        this.opMode = opMode;
        ...
        leftFront.setDirection(Constants.Drivetrain.Direction.LEFT_FRONT);
        rightFront.setDirection(Constants.Drivetrain.Direction.RIGHT_FRONT);
        rightRear.setDirection(Constants.Drivetrain.Direction.RIGHT_REAR);
        leftRear.setDirection(Constants.Drivetrain.Direction.LEFT_REAR);
    ...
    public void setDriveSignal(@NonNull DriveSignal driveSignal) {
        ...
        List<Double> velocities = MecanumKinematics.robotToWheelVelocities(
                driveSignal.getVel(), Constants.Drivetrain.TRACK_WIDTH, Constants.Drivetrain.WHEEL_BASE, Constants.Drivetrain.LATERAL_MULTIPLIER);

        List<Double> accelerations = MecanumKinematics.robotToWheelAccelerations(
                driveSignal.getAccel(), Constants.Drivetrain.TRACK_WIDTH, Constants.Drivetrain.WHEEL_BASE, Constants.Drivetrain.LATERAL_MULTIPLIER);
    ...
    public void setWeightedDrivePower(Pose2d drivePower) {
        ...
        double x = Constants.Drivetrain.Speed.VX_MULTIPLIER * drivePower.getX(); // forward/back
        double y = Constants.Drivetrain.Speed.VY_MULTIPLIER * drivePower.getY(); // strafe
        double heading = Constants.Drivetrain.Speed.OMEGA_MULTIPLIER * drivePower.getHeading(); // turn
    ...
    public void runIteratively() {
        ...
        if (Constants.Drivetrain.USING_FINE_CONTROL) {
        ...
        if (opMode.gamepad1.left_bumper) speed = Constants.Drivetrain.Speed.SLOW_SPEED;
        else if (opMode.gamepad1.right_bumper) speed = Constants.Drivetrain.Speed.FAST_SPEED;
        else speed = Constants.Drivetrain.Speed.NORMAL_SPEED;

        if (opMode.gamepad1.a) resetImu();


        if (Constants.Drivetrain.IS_FIELD_CENTRIC)
